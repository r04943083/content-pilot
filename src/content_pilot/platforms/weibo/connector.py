"""Weibo platform connector implementation."""
...
from content_pilot.platforms.weibo import selectors as sel
...
class WeiboPlatform(AbstractPlatform):
    ...
    async def login(self) -> bool:
        ...
        try:
            ...
            if qr_element:
                logger.info("QR code detected. Please scan with Weibo app.")
            ...
            logger.info("Weibo login successful!")
            return True
    ...
    async def publish_text_image(self, post: PostContent) -> PublishResult:
        ...
        try:
            ...
            if content_el:
                ...
                # Combine title and content for Weibo (no separate title)
            ...
            for tag in post.tags[:5]:
                # Weibo uses #topic# format
                content_el = await page.query_selector(sel.PUBLISH_CONTENT_INPUT)
                if content_el:
                    await content_el.click()
    ...
    async def publish_video(self, post: PostContent) -> PublishResult:
        ...
        try:
            ...
            logger.error("Weibo video publish failed: %s", e)
            return PublishResult(success=False, error=str(e))
        finally:
            await page.close()
    ...
    async def get_post_analytics(self, platform_post_id: str) -> dict:
        ...
        try:
            ...
            logger.error("Failed to get Weibo analytics: %s", e)
            return {}
        finally:
            await page.close()
