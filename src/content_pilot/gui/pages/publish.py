...
STATUSES = ["all", "draft", "approved", "scheduled", "published", "failed"]
    ...
    async def publish_page():
        ...
        with ui.column().classes(
            ...
            with ui.row().classes("full-width items-center justify-between q-mb-md"):
                ui.label(t("publish.post_management")).classes(
                    "text-h6"
                ...
                async def confirm_batch_delete():
                    ...
                    for post_id in list(selected_posts):
                        try:
                            ...
                            pass
                    ui.notify(
                        f"{len(selected_posts)} posts deleted",
                        type="info"
                    )
                    _clear_selection()
                    await refresh_posts()
                ...
                with kanban_container:
                    ...
                    for status, status_posts in status_groups.items():
                        with ui.column().classes("flex-1 q-gutter-sm").style(
                            ...
                            # Posts in this status
                            if not status_posts:
                                ui.label("—").classes("text-grey").classes("text-caption")
                            else:
...
def selectable_post_card(
    ...
    with ui.card().classes("q-pa-md").style(
        ...
        # Store post_id and checkbox on card for batch operations
